...
해결 :  
```<hr />```과 ```<p>``` 태그를 Fragment 태그를 사용하여 감싼다.  
```<hr />```가 ```<p>```의 앞에 있다고 한다면 index가 0보다 클 때만 적용되면 된다. (뒤에 있다면 index가 (배열의 길이 -1) 보다 작을 때!)  
```javascript
import { Fragment } from 'react';
...
JavaScript 함수는 *순수*하다. 컴포넌트를 순수 함수로만 작성하면 당황스러운 버그와 예측할 수 없는 동작을 피할 수 있다.  
<br>

## 순수성이란 무엇인가?
함수형 프로그래밍 세계에서 순수성이란 아래의 2가지 조건을 만족하는 함수이다.
1. It minds its own business. : 함수가 호출되기 전에 존재했던 객체나 변수를 변경하지 않는 것
2. Same inputs, same output. : 같은 값을 넣으면 같은 결과가 나오는 것  <br>

간단히 말하자면 수학 공식이다. $y=x+2$ 라는 식이 있을 때, $x$에 2를 넣으면 무조건 $y$는 4가 나오는 것이다.  
React는 이런 개념을 중심으로 설계되었기 때문에, React에서 **모든 컴포넌트는 순수성을 만족한다고 가정한다.**  
이와 같은 이유로 컴포넌트는 '*레시피*'이고 JSX는 '*요리*'라고 할 수 있다.  
<br>
  
## 순수성을 유지하기 위한 방법
우선, 순수성을 위반하는 사례를 확인해보자.  
```javascript
let guest = 0;
...
// Tea cup for guest #6
```
위의 경우를 보았을 때, 컴포넌트 안에서 전역 변수의 값을 변경하고 있다.  
순수성을 지키기 위한 1번 조건에 위반하였으며, 그 이유로 2번 조건에도 위반하는 결과가 된다.  

> ### Strict Mode란?
> 위의 예시에서 보면 guest 변수에 1을 더해줬음에도 2씩 더해지는 모습을 볼 수 있다.  
> 이것은 순수성을 확인하기 위한 react의 노력이다.  
> Strict Mode는 컴포넌트 함수를 두 번 호출함으로써 이러한 규칙을 위반하는 컴포넌트를 찾아내는 데 도움을 준다.
> 순수 함수는 계산만 하기 때문에 2번 호출해도 바뀌지 않기 때문이다.  

위의 예시를 순수성을 갖춘 함수로 만들기 위해서 guest 값을 props으로 내려주도록 변경한다.  
```javascript
function Cup({ guest }) {
  return <h2>Tea cup for guest #{guest}</h2>;
}
...
## 주의 하자 : Local mutation
~mutation이라는 단어는 겁을 주기 위해 사용한 것 같으니 가장 무서워 보이는 변이로 해석하자.~   
컴포넌트가 렌더링을 하면서 전역에 있는 변수, 즉 컴포넌트 범위 바깥에서 정의된 변수를 변경했다.  
이 점이 주의해야하는 **지역 변이**이다.  
순수 함수는 함수의 범위를 벗어난 변수나 호출 전에 생성된 객체를 변이하지 않아야 한다.  
하지만 컴포넌트가 정의된 함수 안에서 생성한 변수와 객체는 변경해도 된다.  
이것이 바로 *컴포넌트의 작은 비밀*이다.  
<br>
  
## 순수성을 만족해야 하는 이유
내가 컴포넌트를 다루면서 가장 크게 하는 실수는 *순차적으로 실행*될 것이라고 기대하는 것이다.  
컴포넌트는 특정한 순서대로 렌더링 되는 것이 아니라 각각 *독립적으로 실행*된다.  
그러므로 각 컴포넌트 사이에 의존성없이 작성해야 한다.  
- 입력이 변경되지 않은 컴포넌트는 렌더링 건너뛰기를 통해 성능을 향상시킨다. 순수 함수는 항상 동일한 결과를 반환하므로 캐싱해도 안전하다.
- 깊은 컴포넌트 트리를 렌더링하는 도중에 일부 데이터가 변경되면 React는 오래된 렌더링을 완료하기 위해 시간을 낭비하지 않고 렌더링을 다시 시작할 수 있다. 순수성 덕분에 언제든지 계산을 중단할 수 있다. <br>

단, 이벤트 핸들러의 경우 컴포넌트 내부에 정의되지만 렌더링 중에는 실행하지 않고 사용자의 동작에 반응한다.
이런 이유로 이벤트 핸들러는 순수성을 유지 하지 않아도 된다.
<br>
  
## Keeping Components Pure Recap (컴포넌트 순수성 유지하기 요약)
- 컴포넌트는 순수해야 한다.
  + 렌더링 전에 존재했던 객체나 변수를 변경하지 않아야 한다.
  + 동일한 입력이 주어지면 컴포넌트는 항상 동일한 JSX를 반환해야 한다.
- 렌더링은 언제든지 발생할 수 있으므로, 컴포넌트는 서로의 렌더링 순서에 의존해서는 안 된다.
- 컴포넌트가 렌더링에 사용하는 어떠한 입력값도 변이해서는 안 된다.(props, state 및 context가 포함) 화면을 업데이트하려면 기존 객체를 변이하는 대신 “set” state를 사용하자.
- 컴포넌트의 로직을 반환하는 JSX 안에 표현하기 위해 노력해보자. “무언가를 변경”해야 할 때는 보통 이벤트 핸들러에서 이 작업을 수행하고자 할 것이다. 최후의 수단으로 useEffect를 사용할 수도 있다.
- 순수 함수를 작성하는 데는 약간의 연습이 필요하지만, React 패러다임의 힘을 발휘할 수 있다.
<br>
  
## Keeping Components Pure Try out some challenges (컴포넌트 순수성 유지하기 도전 과제)
### Q1. Fix a broken clock
문제 :  
...
해결 :  
return문의 JSX에 접근하려고 하지 말고 className을 변수로 구해서 넣어주었다.  
```javascript
...
This is a bug. Find the cause of the bug and fix it.  
  
해결 :  
문제의 Profile.js에서 컴포넌트에서 변경하는 currentPerson이라는 변수는 전역 변수였기 때문에 일어난 문제이다.  
props로 받는 person의 정보만을 자식에게 넘겨주어 해결했다.  
```javascript
// App.js
import Profile from "./Profile.js";
...
해결 :  
문제의 코드를 봤을 때 시간이 1초 흐를 때마다 랜더링되면서 ```Create Story```가 잔뜩 생기는 것을 볼 수 있다.  
StoryTray를 품고 있는 부모 컴포넌트가 1초마다 재랜더링이 일어나면서 stories라는 변수에 직접 push를 했기 때문에 계속 쌓이는 것이다.  
이 문제를 해결하기 위해 stories를 ```slice()``` 메소드를 통해 복사하여 새로 만들어진 변수에 ```Create Story```를 push한다.
```javascript
// StoryTray.js
